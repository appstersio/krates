...
describe HostNode do
  ...
  context 'when the node exists' do
    ...
    it 'does not allow duplicate names' do
      expect{HostNode.create!(grid: grid, node_number: 2, name: 'node-1')}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: kontena_(test|development).host_nodes index/)
    end

    it 'does not allow duplicate node numbers' do
      expect{HostNode.create!(grid: grid, node_number: 1, name: 'node-2')}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: kontena_(test|development).host_nodes index/)
    end
  end
