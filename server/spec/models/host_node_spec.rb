...
describe HostNode do
  ...
  context 'when the node exists' do
    ...
    it 'does not allow duplicate names' do
      expect{HostNode.create!(grid: grid, node_number: 2, name: 'node-1')}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: krates_(test|development).host_nodes index/)
    end

    it 'does not allow duplicate node numbers' do
      expect{HostNode.create!(grid: grid, node_number: 1, name: 'node-2')}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: krates_(test|development).host_nodes index/)
    end
  end
  ...
  describe '#token' do
    ...
    context 'with a node that has a node token' do
      ...
      it 'does not allow multiple nodes to share the same token' do
        expect{HostNode.create!(grid: grid, name: 'node-2', node_number: 2, token: token)}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: krates_(test|development).host_nodes index: token_1 dup key: { : "asdfasdfasdfasdf" }/)
      end
      ...
      context 'with a second grid' do
        ...
        it 'does not allow nodes to share the same token' do
          expect{HostNode.create!(grid: grid2, name: 'node-2', node_number: 2, token: token)}.to raise_error(Mongo::Error::OperationFailure, /E11000 duplicate key error collection: krates_(test|development).host_nodes index: token_1 dup key: { : "asdfasdfasdfasdf" }/)
        end
      end
    end
  end
