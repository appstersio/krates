...
describe 'krates service shutdown' do
  ...
  context 'for a http test service' do
    ...
    context "using ungraceful shutdown to fail requests" do
      before(:all) do
        with_fixture_dir('stack/shutdown-test') do
          run! "krates stack install -n shutdown-test-fail -v trap=false"
        end
      end
      after(:all) do
        run! 'krates stack rm --force shutdown-test-fail'
      end

      describe "after re-deploying the server" do
        before do
          run! 'krates service deploy --force shutdown-test-fail/server'
        end
    ...
    context "using graceful shutdown to close the listening socket" do
      before(:all) do
        with_fixture_dir('stack/shutdown-test') do
          run! "krates stack install -n shutdown-test-graceful -v graceful_shutdown=true"
        end
      end
      after(:all) do
        run! 'krates stack rm --force shutdown-test-graceful'
      end

      describe "after re-deploying the server" do
        before do
          run! 'krates service deploy --force shutdown-test-graceful/server'
        end
    ...
    context "using graceful shutdown without a healthcheck" do
      before(:all) do
        with_fixture_dir('stack/shutdown-test') do
          run! "krates stack install -n shutdown-test-nohealthcheck -v graceful_shutdown=true -v healthcheck=false"
        end
      end
      after(:all) do
        run! 'krates stack rm --force shutdown-test-nohealthcheck'
      end

      describe "after re-deploying the server" do
        before do
          run! 'krates service deploy --force shutdown-test-nohealthcheck/server'
        end
    ...
    context "using delayed shutdown without closing the listening socket" do
      before(:all) do
        with_fixture_dir('stack/shutdown-test') do
          run! "krates stack install -n shutdown-test-delay -v graceful_shutdown=false"
        end
      end
      after(:all) do
        run! 'krates stack rm --force shutdown-test-delay'
      end

      describe "after re-deploying the server" do
        before do
          run! 'krates service deploy --force shutdown-test-delay/server'
        end
