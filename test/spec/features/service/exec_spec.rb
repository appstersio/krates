...
describe 'service exec' do
  ...
  it 'runs a command inside a service' do
    k = kommando("krates service exec test-1 -- hostname -s")
  ...
  it 'exits with error if command fails' do
    k = kommando("krates service exec test-1 -- ls -l /nonexist")
    k.run
  ...
  it 'exits with command error' do
    k = kommando("krates service exec --shell test-1 exit 32")
    k.run
    expect(k.code).to eq(32)
  end

  it 'returns an error if command not found' do
    k = kommando("krates service exec test-1 thisdoesnotexist")
    k.run
  ...
  describe '--interactive' do
    it 'runs a command with piped stdin' do
      k = kommando("$ echo beer | krates service exec -i test-1 rev")
      expect(k.run).to be_truthy
      expect(k.code).to be_zero, k.out
    ...
    it 'runs a command with piped non-ascii stdin' do
      k = kommando("$ echo f\u00e5\u00e5 | krates service exec -i test-1 -- sh -c 'LANG=C.UTF-8 rev'")
      expect(k.run).to be_truthy
      expect(k.code).to be_zero, k.out
  ...
  context 'with multiple instances' do
    before(:all) do
      run("krates service scale test-1 2")
    end

    after(:all) do
      run("krates service scale test-1 1")
    end

    it 'runs a command inside a service on a given instances' do
      k = kommando("krates service exec --instance 2 test-1 -- hostname -s")
      expect(k.run).to be_truthy
      expect(k.code).to be_zero, k.out
    ...
    it 'runs a command on every instance with --all' do
      k = kommando("krates service exec --all --silent test-1 -- hostname -s")
      expect(k.run).to be_truthy
      expect(k.code).to be_zero, k.out
      expect(k.out).to eq("test-1-1\r\ntest-1-2\r\n")
    end
    ...
    it 'fails early if running a command on every instances with --all fails' do
      k = kommando("krates service exec --all --silent --shell test-1 -- hostname -s && false")
      expect(k.run).to be_truthy
      expect(k.code).to_not be_zero, k.out
      expect(k.out).to eq("test-1-1\r\n")
    end
    ...
    it 'keeps going if running a command on every instances with --all --skip' do
      k = kommando("krates service exec --all --skip --silent --shell test-1 -- hostname -s && false")
      expect(k.run).to be_truthy
      expect(k.code).to_not be_zero, k.out
      expect(k.out).to eq("test-1-1\r\ntest-1-2\r\n")
    end
