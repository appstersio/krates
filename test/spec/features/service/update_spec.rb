describe 'service update' do
  before(:each) do
    run("krates service create test-1 nginx:1-alpine")
  end
  ...
  context 'health check' do
    it 'updates health check' do
      run! "krates service update --health-check-port 8080 --health-check-protocol http --health-check-uri / test-1"
    ...
    it 'allows to remove health check' do
      run "krates service update --health-check-port 8080 --health-check-protocol http --health-check-uri / test-1"
      k = run! "krates service update --health-check-port none --health-check-protocol none test-1"
  ...
  context 'stop_grace_period' do
    it 'allows update stop-timeout' do
      run! "krates service update --stop-timeout 1m23s test-1"
      k = run! "krates service show test-1"
