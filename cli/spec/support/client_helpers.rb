module ClientHelpers
  ...
  def self.included(base)
    ...
    base.before(:each) do
      ...
      allow(File).to receive(:exist?).with(File.join(Dir.home, '.krates.json')).and_return(true)
      allow(File).to receive(:readable?).with(File.join(Dir.home, '.krates.json')).and_return(true)
      allow(File).to receive(:read).and_call_original
      allow(File).to receive(:read).with(File.join(Dir.home, '.krates.json')).and_return(JSON.dump(settings))
